    ...
    <main class="relative grid min-h-screen place-items-center overflow-hidden px-4 py-10">
        ...
        <section class="relative w-full max-w-md rounded-[24px] border border-white/10 bg-black/80 p-6 shadow-2xl shadow-black/70 backdrop-blur sm:p-8">
            ...
            <div class="mt-8 text-center">
                <h1 class="font-orbitron text-2xl font-black tracking-tight text-white">Entrar na plataforma</h1>
                <p class="mt-2 text-sm leading-6 text-zinc-400">Use seu e-mail e senha para continuar.</p>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-5">
                ...
                <div>
                    ...
                    @error('email')
                        <p class="mt-2 rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-2 text-sm leading-5 text-red-200">Não consegui liberar o acesso. Confira o e-mail e a senha.</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @error('password')
                        <p class="mt-2 rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-2 text-sm leading-5 text-red-200">Informe a senha para continuar.</p>
                    @enderror
                </div>

                <div class="flex items-center justify-between gap-4">
                    <a href="{{ route('signup.create') }}" class="text-sm font-bold text-zinc-300 transition hover:text-yellow-200 focus:outline-none focus:ring-4 focus:ring-yellow-300/20">
                        Criar cadastro
                    </a>
                    <a href="{{ route('password.request') }}" class="text-sm font-bold text-yellow-300 transition hover:text-yellow-200 focus:outline-none focus:ring-4 focus:ring-yellow-300/20">
