...
<head>
    ...
    <title>Nova senha - GarageON</title>
    ...
    <main class="relative grid min-h-screen place-items-center overflow-hidden px-4 py-10">
        ...
        <section class="relative w-full max-w-md rounded-[24px] border border-white/10 bg-black/80 p-6 shadow-2xl shadow-black/70 backdrop-blur sm:p-8">
            ...
            <div class="mt-8 text-center">
                <h1 class="font-orbitron text-2xl font-black tracking-tight text-white">Criar nova senha</h1>
                <p class="mt-2 text-sm leading-6 text-zinc-400">Escolha uma senha segura para continuar.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.update') }}" class="mt-8 space-y-5">
                ...
                <div>
                    ...
                    <div class="mt-2 rounded-xl border border-white/10 bg-[#111111] transition focus-within:border-yellow-300 focus-within:shadow-[0_0_0_4px_rgba(255,196,0,.10)]">
                        <input id="email" name="email" type="email" value="{{ old('email', $email) }}" required autocomplete="email" placeholder="contato@empresa.com" class="w-full rounded-xl bg-transparent px-4 py-3.5 text-white outline-none placeholder:text-zinc-600">
                    </div>
                    @error('email')
                ...
                <div>
                    <label for="password" class="text-sm font-bold text-zinc-100">Nova senha</label>
                    <div class="mt-2 rounded-xl border border-white/10 bg-[#111111] transition focus-within:border-yellow-300 focus-within:shadow-[0_0_0_4px_rgba(255,196,0,.10)]">
                        <input id="password" name="password" type="password" required autocomplete="new-password" placeholder="Digite a nova senha" class="w-full rounded-xl bg-transparent px-4 py-3.5 text-white outline-none placeholder:text-zinc-600">
                    </div>
                    @error('password')
                        <p class="mt-2 rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-2 text-sm leading-5 text-red-200">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="mt-2 rounded-xl border border-white/10 bg-[#111111] transition focus-within:border-yellow-300 focus-within:shadow-[0_0_0_4px_rgba(255,196,0,.10)]">
                        <input id="password_confirmation" name="password_confirmation" type="password" required autocomplete="new-password" placeholder="Repita a nova senha" class="w-full rounded-xl bg-transparent px-4 py-3.5 text-white outline-none placeholder:text-zinc-600">
                    </div>
                </div>
